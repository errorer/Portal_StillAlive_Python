...
def move(x, y, update_cursor=True, mutex=True):
    ...
    global print_lock
    if(mutex):print_lock.acquire()
    print("\033[%d;%dH" % (y, x), end='')
    sys.stdout.flush()
    if(update_cursor):
        cursor_x = x
        cursor_y = y
    if(mutex):print_lock.release()
...
class lyric:
    def __init__(s, _words, _time, _interval, _mode):
        '''
...
lyrics = [
        ...
        lyric("\00\00\00\00\00\00\00",              400,    -1,   0), #Keep flushing the buffer
        ...
        ##########  Page 3  ##########
        lyric(0,                                    10603,  0,    3), #Clear lyrics
        ...
        ##########  Page 4  ##########
        lyric(0,                                    15697,  0,    3), #Clear lyrics
        lyric("",                                   15757,  0.05, 0),
        lyric("",                                   15757,  0.05, 0),
        lyric("",                                   15757,  0.05, 0),
        ...
        lyric(0,                                    17900,  0,    3), #Clear lyrics
        lyric(0,                                    18500,  0,    3), #Clear lyrics
...
class thread_credits (threading.Thread):
    def run(self):
        ...
        for ch in credits:
            ...
            if(ch == '\n'):
                credit_x = 0
                for j in range(len(last_credit), 33):last_credit += " "
                print_lock.acquire()
...
while(lyrics[currentLyric].mode != 9):
    ...
    if(currentTime > lyrics[currentLyric].time):
        ...
        if(lyrics[currentLyric].mode == 0):
            ...
            x = 0
            y = 0
        elif(lyrics[currentLyric].mode == 4):playsound.playsound('sa1.mp3', False)
